...
void run_dsmc_pump(mesh_set & mesh, fmatrix & dens_n, configuration & config, string output_name){
    ...
    const double pump_prob = 0.6;
    const double boundary_roughness = 0.5;
    
    // const double v_flux = sqrt(config.f("physical/q") * t_neutral * 8 / (config.f("physical/pi") * m_i));
    // const double pump_area = config.f("geometry/l_x") + config.f("geometry/l_y");
    // double pump_speed = 5000 * 1e-3;
